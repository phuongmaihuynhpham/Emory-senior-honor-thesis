...
nregions = 4;
[A_true, A_int, b_true, x_true, C, d] = TestSetup4(nregions);
%
...
T = table(norm(x_true - A_true\b_true)/norm(x_true), ...
    norm(x_true - A_int\b_true)/norm(x_true), ...
    norm(x_true - A_true\b)/norm(x_true), ...
    norm(x_true - x1)/norm(x_true), ...
    norm(x_true - x2)/norm(x_true), ...
    norm(A_true - A_int,'fro')/norm(A_true, 'fro'));
T.Properties.VariableNames = {'\, true A and b', ...
    '\. RBF approx of A, noise free b', ...
    '\, true A, noisy b', ...
    'Hybrid LSQR, true A, noisy b', 'Hybrid LSQR, RBF approx of A, noisy b',...
    'Norm int A and true A'};
display(T)
A(1,1) = norm(A_true - A_int,'fro')/norm(A_true, 'fro');
